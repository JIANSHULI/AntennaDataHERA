...
for Growth_Rate in Growth_Rate_List:
	for Outer_Diameter in Outer_Diameter_List:
		for Band_Resistance in Band_Resistance_List:
			...
			p.xlabel('delay (ns)')
			p.legend(loc='best')
			p.title('S11_CST_Delay_0.%i-%i-%i_NF_PW%i_band-%s' %(Growth_Rate, Inner_Diameter,Outer_Diameter,S11_Power,Band_Resistance))
			#p.show()
			p.grid()
			#p.savefig('../plots/s11_CST_vs_ReflectometryRich_TallCylinderGapFeedOnly_Delay.pdf',bbox_inches='tight')
			p.savefig('/Users/JianshuLi/Documents/Miracle/Research/Cosmology/21cm Cosmology/Results/Sinuous_Antenna/Plots/S11_CST_Delay_0.%i-%i-%i_NF_PW%i_band-%s.pdf'%(Growth_Rate, Inner_Diameter,Outer_Diameter,S11_Power,Band_Resistance),bbox_inches='tight')
			p.close()
			...
			p.xlabel('f (GHz)')
			p.legend(loc='best')
			p.title('S11_CST_Frequency_0.%i-%i-%i_NF_PW%i_band-%s' %(Growth_Rate, Inner_Diameter,Outer_Diameter,S11_Power,Band_Resistance)) 
			#p.show()
			p.grid()
			#p.savefig('../plots/s11_CST_vs_ReflectometryRich_TallCylinderGapFeedOnly_Frequency.pdf',bbox_inches='tight')
			p.savefig('/Users/JianshuLi/Documents/Miracle/Research/Cosmology/21cm Cosmology/Results/Sinuous_Antenna/Plots/S11_CST_Frequency_0.%i-%i-%i_NF_PW%i_band-%s.pdf'%(Growth_Rate, Inner_Diameter,Outer_Diameter,S11_Power,Band_Resistance),bbox_inches='tight')
			p.close()